    ...
    return (
        <div className="animate-fadeIn">
            <div className="page-header" style={{ alignItems: 'flex-start' }}>
                <div>
                    <h1>Dashboard</h1>
                    <p className="subtitle">Restaurant overview and analytics</p>
                </div>
            ...
            <div className="grid grid-4" style={{ marginBottom: 'var(--space-lg)' }}>
                ...
                <div className="stat-card" style={{ background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(139,92,246,0.2)' }}>
                    <div className="stat-icon" style={{ background: 'rgba(139,92,246,0.15)' }}>🍎</div>
                    <div className="stat-info">
                        <h3>Total Menus</h3>
                        <div className="stat-value" style={{ background: 'var(--gradient-secondary)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                            {report?.menuItemsCount || 0}
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-3" style={{ marginBottom: 'var(--space-xl)' }}>
                ...
                <div className="card" style={{ 
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        <span className="badge badge-warning" style={{ marginBottom: 'var(--space-sm)' }}>✨ Star of the Month</span>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--font-sm)', marginBottom: 'var(--space-md)' }}>
                            Fastest selling product in this period
                        </p>
                ...
                <div className="card" style={{ gridColumn: 'span 2' }}>
                    <h3 style={{ fontWeight: 700, marginBottom: 'var(--space-md)' }}>🔥 Top Selling Items</h3>
                            ...
                            report.topItems.slice(0, 8).map((item, i) => (
                                <div key={i} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                                        ...
                                        <span style={{ fontWeight: 500, fontSize: 'var(--font-sm)' }}>{item.name}</span>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontSize: 'var(--font-xs)', color: 'var(--text-muted)' }}>{item.count} orders</div>
                                        <div style={{ fontSize: 'var(--font-sm)', fontWeight: 700, color: 'var(--accent-primary)' }}>{formatCurrency(item.revenue)}</div>
                                    </div>
                                </div>
                            ))
                        )}
            ...
            <div className="grid grid-2" style={{ marginBottom: 'var(--space-xl)' }}>
                ...
                <div className="card">
                    <h3 style={{ fontWeight: 700, marginBottom: 'var(--space-md)' }}>📊 Order Breakdown</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
                        {Object.entries(report?.ordersByType || {}).map(([type, count]) => {
                            const total = report?.totalOrders || 1;
                            const pct = ((count / total) * 100).toFixed(0);
                            const colors = { dine_in: 'var(--info)', takeaway: 'var(--warning)', delivery: 'var(--success)' };
                            return (
                                <div key={type}>
                                    ...
                                    <div style={{ height: 8, background: 'rgba(255,255,255,0.05)', borderRadius: 'var(--radius-full)', overflow: 'hidden' }}>
                                        <div style={{ width: `${pct}%`, height: '100%', background: colors[type] || 'var(--accent-primary)', borderRadius: 'var(--radius-full)', transition: 'width 0.5s ease' }}></div>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div className="card">
                    <h3 style={{ fontWeight: 700, marginBottom: 'var(--space-md)' }}>🕒 Recent Orders</h3>
                        ...
                        {(report?.orders || []).slice(0, 5).map(order => (
                            <div key={order._id} style={{ 
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{new Date(order.createdAt).toLocaleTimeString()}</div>
                                </div>
                                <div style={{ textAlign: 'right' }}>
                                    <div style={{ fontSize: 'var(--font-sm)', fontWeight: 700 }}>{formatCurrency(order.total)}</div>
                                    <span className={`badge ${order.status === 'delivered' ? 'badge-success' : 'badge-warning'}`} style={{ fontSize: 10, padding: '1px 6px' }}>
                                        {order.status}
                                    </span>
                                </div>
                            </div>
